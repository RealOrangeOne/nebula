...
func (lh *LightHouse) addRemoteV4(vpnIP uint32, to *Ip4AndPort) {
	...
	if l.Level >= logrus.DebugLevel {
		l.WithField("remoteIp", ip).WithField("allow", allow).Debug("remoteAllowList.Allow")
	}
...
func (lh *LightHouse) SendUpdate(f EncWriter) {
	...
	for vpnIp := range lh.lighthouses {
		...
		if err != nil && l.Level >= logrus.DebugLevel {
...
func (lhh *LightHouseHandler) handleHostQuery(n *NebulaMeta, vpnIp uint32, addr *udpAddr, w EncWriter) {
	...
	if !lhh.lh.amLighthouse {
		if l.Level >= logrus.DebugLevel {
			l.Debugln("I don't answer queries, but received from: ", addr)
		}
		return
	}
	...
	if err != nil {
		l.WithError(err).WithField("vpnIp", IntIp(vpnIp)).Error("Failed to marshal lighthouse host was queried for")
		return
	}
...
func (lhh *LightHouseHandler) handleHostUpdateNotification(n *NebulaMeta, vpnIp uint32) {
	if !lhh.lh.amLighthouse {
		if l.Level >= logrus.DebugLevel {
			l.Debugln("I am not a lighthouse, do not take host updates: ", vpnIp)
		}
		return
	}
...
func (lhh *LightHouseHandler) handleHostPunchNotification(n *NebulaMeta, vpnIp uint32, w EncWriter) {
	...
	punch := func(vpnPeer *udpAddr) {
		...
		if l.Level >= logrus.DebugLevel {
			...
			l.Debugf("Punching on %d for %s", vpnPeer.Port, IntIp(n.Details.VpnIp))
		}
	}
